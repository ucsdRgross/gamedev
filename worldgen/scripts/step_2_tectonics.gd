...
func execute(gen: WorldGenerator, settings: WorldSettings) -> void:
	...
	blue.set_shader_parameter("seed", settings.main_seed + 15)
	...
	blue.set_shader_parameter("warp_frequency", settings.warp_frequency)
	...
	deform.set_shader_parameter("seed", settings.main_seed + 15)  # must match blueprint
	...
	deform.set_shader_parameter("warp_frequency", settings.warp_frequency)
