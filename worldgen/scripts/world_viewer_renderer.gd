...
func _draw_grid_vector_overlays() -> void:
	...
	if generator.snapshots.has("Tectonics_Debug"):
		_draw_vector_layer(generator.snapshots["Tectonics_Debug"], "Tectonics_Debug", Vector2(sub_w * 2, 0), scale)
	if generator.snapshots.has("Cities"):
		_draw_vector_layer(generator.snapshots["Cities"], "Cities", Vector2(sub_w, 2 * sub_h), scale)
	if generator.snapshots.has("Graph"):
		_draw_vector_layer(generator.snapshots["Graph"], "Graph", Vector2(sub_w * 2, 2 * sub_h), scale)
...
func _draw_ui_legend(step: String) -> void:
	...
	if step in ["Landmass", "Tectonics", "PeaksAndValleys", "Erosion"]:
		...
		items = [
			{"c": Color.html("#f43f5e"), "n": "Land Crust Plates"},
			{"c": Color.html("#0ea5e9"), "n": "Ocean Crust Plates"}
		]
