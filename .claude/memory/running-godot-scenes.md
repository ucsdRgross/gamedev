...
- **One run at a time.** ⚠ Overlapping runs **FABRICATE FAILURES in unrelated suites** — they share
  ...
  background batch. Two SESSIONS on one box share `user://` through `%APPDATA%`: export a private
  `APPDATA` per session (measured: two shared runs truncated each other's logs and pushed a full
  run past the wrapper's timeout). The tier and suite counts above are derived from the scene,
  never trusted from a doc.
