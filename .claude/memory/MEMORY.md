...
- [Spotlight design + phase 1](solatro-spotlight-design.md) — design CONFIRMED 2026-08-03; **phase 1 (the mechanical spotlight: `active`→`spotlit`, `forced_spotlight`, `ScoringSection`, hand re-evaluation) IS SHIPPED AND GREEN 2026-08-04**, phases 2–4 pending; GAP-001 open; status ledger is solatro/HANDOFF_spotlight.md
...
- [Resuming a design round: read the log](design-round-read-the-log.md) — `answers.log` `override:true` is a branch to author even when `answers.json` shows a clean option; add options and widen gates, never replace
