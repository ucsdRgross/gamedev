...
description: "Eight ways a test passes while asserting nothing, and the red-then-green rule that catches all of them"
metadata:
  type: feedback
---

A green suite is the weakest evidence there is. One run produced **eight** tests that passed while
proving nothing, each of which looked fine in review:
...
**The rule that catches all eight: prove every new test red-then-green.** Neutralise the behaviour,
