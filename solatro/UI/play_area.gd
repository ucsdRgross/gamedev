...
#Emitted once a rebuild's CardVisuals are all in-tree and _ready. They add_child via call_deferred,
#so right after set_card_zones they are mapped in data_card but not yet ready; a deferred emit
#queued after those adds (FIFO) fires only once they have entered the tree.

#Lets a caller that must animate a freshly built board await instead of poll. Pair with
#`visuals_ready()` for the already-ready case.
...
#⚠ THE RESERVE ARRIVES AFTER THE SHOW HAS ALREADY OPENED. `GameView` reads the HUD's authored
#offsets at the END of its own `_ready()`, by which time a focused grid has already been fitted
#against an inset of zero. Re-fitting here is what makes the arriving reserve reach the zoom.

#⚠ THE RECT FIRST, UNCONDITIONALLY. `_zoom_board_to()` early-returns when the zoom is unchanged,
#and a reserve that arrives without moving the zoom is exactly that case — the scroller would keep
#the offsets it took when it thought it had the whole screen.
...
#The picture x the board's current pan puts under the LEFT edge of the grid the view is centred on
#-- the same value the Entrance aligns to. Exposed so anything OUTSIDE the scroll can ride the
#identical pan rather than a second, independent measure of where the view rests.

#The board's scroll window spans the whole picture, so this is a LIVE layout position: the grid
...
#is_instance_valid guard: a board rebuild can free the control this still points at, and a freed
#entry in a typed dictionary is an engine error.
...
#IN THE OVERVIEW, ENTER FOCUSES THE SELECTED GRID even when nothing on the board holds focus, since
#an arrow selection must be committable on its own. With a board control focused the pass below
#already does it, and leaving that case alone keeps a focused ENTRANCE card selectable.
...
#Held cards ride ABOVE all resting cards, still below PropLayer. move_child to the end of the card's
#OWN layer, Entrance or grid — no z_index, structural order only (LAYERING.md). ungrab_cards then
#rebuilds row-major order.
...
#child 0 = the zone/type header (z == -1)
	var idx := v.z + 1
...
#⚠ THE UNIFORM PITCH IS NOT THE WHOLE STORY, AND EVERY PROP ANCHORS TO THIS. The reveal grows one
#layer's strip, lifting every layer above it by an amount the pitch does not describe. Still pure
#math: the offset comes from the same eased numbers that size the controls.
...
#This rebuild satisfies any queued request, and the focus panel is dismissed because the control it
#anchored to may be about to move or be freed.
...
#visual pass anyway.
	...
	if not game: return
	var game_state := game.state
#Sizing, style overrides and focus logic per zone, then ONE structural ordering pass over both
#zones (row-major — see _order_board_cards). Upper zone first, lower second, so lower-zone cards
#draw over upper.
...
#Structural draw order (no z_index anywhere, LAYERING.md), ROW-MAJOR across columns (owner spec):
#per zone the type/zone headers first, then row 0 of every column, then row 1, upper zone before
#lower. Cards only overlap WITHIN a column, so they render exactly as column-major did.

#Row-major is what makes each row CONTIGUOUS in CardLayer: a split prop brackets a whole ROW, its
#back half before the row's first card and its front half after its last. See
#PropLayer._apply_split.

#Index-safe by construction: targets are assigned 0,1,2,… ascending and only to visuals verified in
#the layer at this moment, so `desired` can never exceed the child count. Ascending order also
#converges in ONE pass, and a still board does zero move_childs.

#Freshly created CardVisuals add_child via call_deferred and are skipped; they append in creation
#order and the next rebuild slots them. Held cards keep their lifted end-of-layer spot.

#⚠ TWO LAYERS, TWO INDEPENDENT ORDERINGS. A move_child index only means anything inside the layer
#holding the child. The Entrance has its own EntranceCardLayer, so sharing one ordered list with
#the grids made a correctly-parented visual read as a never-landing add and requeued every frame.
...
#Card inspector for EVERY input mode: a mouse hover grabs focus too, so focus is the one unified
#hover signal.

#⚠ Two gates, and they are different questions. In Info mode this panel ALWAYS yields, because the
#wall's card is the one description system and two panels describing one card is what a single info
#card replaced. Outside Info mode `wall_screen_popups` decides whether a description exists at all.
...
#Mouse hover grabs focus, so focus covers mouse, keyboard and controller alike
#([[solatro-multimodal-input]]). The text is the localized ControlCard.describe_card.

#⚠ Deliberately NOT Control.tooltip_text: the native tooltip is a popup Window that sat under the
#cursor and blocked board clicks. This panel is pure display (MOUSE_FILTER_IGNORE everywhere, focus
#NONE) and can never touch input.

#A PERMANENT child of OverlayLayer, re-pinned beside its anchor every frame so a container relayout
#cannot strand it. Mouse-exit, ui_cancel, ungrab and rebuild all dismiss it.
const FOCUS_INFO_WIDTH := 260.0
const FOCUS_INFO_GAP := 4.0

var _focus_info : PanelContainer = null
var _focus_info_label : Label = null
## The board control the panel is pinned beside.
var _focus_info_anchor : Control = null
...
#No z_index: OverlayLayer is the last sibling of TopLevelVBox, so its children draw above every
#card and prop by tree order (LAYERING.md).
...
#CRITICAL: SmoothScrollContainer force-rewrites every Control added under it to MOUSE_FILTER_PASS,
#which turned this panel into a mouse hit-target over the cards and BLOCKED board clicks. It skips
#nodes already carrying its meta marker, so claim the marker BEFORE entering the tree.
...
#Whether the wall's Info mode is on. Read through `WallPicture.settings()` — the one accessor that
#answers "which PlayerSettings" — so a tool previewing the board sees its own knobs.
...
#A card's `InfoEntry` for the wall's info card. The TEXT is `ControlCard.describe_card()`, the same
#string the in-screen inspector shows so the two cannot drift; the VISUAL is a real preview card
#built through `CardsViewer`, the same listing `MapHoverPanel` uses for booster previews.
...
#keep the panel pinned to its anchor while visible
	set_process(true)

#Pin the panel beside its anchor control, flipping left or lifting up when it would leave the area.
#Global placement is safe every frame: the panel and the anchor both live in the scroll content, so
#their globals move in lockstep under scrolling.
...
#the control it anchored to was freed by a rebuild
		hide_focus_info()
		return
...
#Entrance's fanned strips and its score gutter have to be pushed by hand.
	var hbox : HBoxContainer = upper_zone_right
	if hbox:
#⚠ THIS FOLLOWS THE ENTRANCE'S REVERSED ORDER, AND IT IS THE LAST WRITER OF THOSE HEIGHTS. Child 0
#is the newest card and shows whole, each card under it shows one depth pitch, and the slot's own
...
#⚠ Both consumers have to be idle before processing stops, or whichever finishes first switches the
#other one off mid-animation.
	if not revealing and _focus_info_anchor == null: set_process(false)

func hide_focus_info() -> void:
	_focus_info_anchor = null
#⚠ ONLY IF THE REVEAL IS ALSO IDLE, AND ONLY IF NO DEPTH LAYER IS STILL ARRIVING. `_process` drives
#both easings, and stopping it unconditionally froze a growing row at whatever fraction it had
#reached. Measured: a row stuck at 54 against a container at 74, with an entry that never cleared.
	if _row_open.is_empty() and _layer_grown.is_empty():
#nothing to pin while hidden
		set_process(false)
