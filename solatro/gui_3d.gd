...
func _mouse_input_event(_camera: Camera3D, event: InputEvent, event_position: Vector3, _normal: Vector3, _shape_idx: int) -> void:
	...
	var quad_mesh_size := (node_quad.mesh as QuadMesh).size
	...
	if event is InputEventScreenTouch:
		(event as InputEventScreenTouch).position = event_pos2D
	if event is InputEventMouse:
		(event as InputEventMouse).position = event_pos2D
		(event as InputEventMouse).global_position = event_pos2D

	# Calculate the relative event distance.
	if event is InputEventMouseMotion or event is InputEventScreenDrag:
		(event as InputEventMouseMotion).position = event_pos2D
		...
		# the previous position from the new position. This will give us the distance the event traveled from prev_pos.
			...
			(event as InputEventMouseMotion).relative = event_pos2D - last_event_pos2D
			(event as InputEventMouseMotion).velocity = (event as InputEventMouseMotion).relative / (now - last_event_time)
