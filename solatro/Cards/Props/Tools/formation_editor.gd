...
@export var mode : PropFormationData.Mode = PropFormationData.Mode.ORDERED:
	set(value):
		...
		var f := _formation()
		if f: f.mode = mode
## The current formation's points, UNSCALED card space (edit freely; points outside the
## card footprint draw RED — every point must stay inside one card).
...
## Stand-in for the game's card_scale: scales the card footprint AND the point offsets —
## but NOT the prop art, exactly like in game (PropLayer never scales visuals; only
## lane_offset is multiplied by card_scale). Set to your live card_scale for exact parity.
@export var preview_scale : float = 2.0
...
## Distance between column anchors (unscaled) — the play area's slot pitch stand-in.
@export var column_pitch : float = CardVisual.CARD_SIZE.x + 10.0
...
func _pull_points() -> void:
	var f := _formation()
	if f: mode = f.mode   # setter pushes back (no-op round trip)
	points = f.points.duplicate() if f else PackedVector2Array()   # setter pushes back + redraws

func _push_points() -> void:
	var f := _formation()
	if f: f.points = points.duplicate()

func _add_formation() -> void:
	if _set == null: _set = PropFormationSet.new()
	var f := PropFormationData.new()
	f.mode = mode
	f.points = points.duplicate() if not points.is_empty() else PackedVector2Array([Vector2.ZERO])
...
func _generate() -> void:
	...
	for i : int in out.size():
		...
		out[i] = out[i].clamp(-CARD * 0.5, CARD * 0.5)   # a formation stays inside ONE card
...
func _spawn_preview() -> void:
	...
	while remaining > 0 and col < 32:
		...
		var offsets := _set.offsets_for(n, seed_value)
		for i : int in n:
			# Scale positions only, NEVER the prop art — game parity (PropLayer adds visuals
			# unscaled; only lane_offset gets card_scale).
			var vis := _make_prop()
			add_child(vis)
...
func _draw() -> void:
	...
	# Editable points of the CURRENT formation on column 0, with indices; out-of-card = RED.
	var font := ThemeDB.fallback_font
	for i : int in points.size():
		var inside : bool = absf(points[i].x) <= CARD.x * 0.5 and absf(points[i].y) <= CARD.y * 0.5
