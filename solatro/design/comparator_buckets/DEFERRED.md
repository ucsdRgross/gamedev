...
|---|---|---|---|
| ~~**E1**~~ | ~~A benchmark for the scoring path~~ — **DONE.** `Tests/Engine/scoring_cost.tscn` times `PokerHands.score` per scored line: empty-hand floor, warm-up, averaged, rows for 5/8/13/30 cards with and without rules, plus a SCORE IMPACT table (what a rule is worth, not what it costs — the arithmetic half of "is a merging card broken"). Not in `all_tests.tscn` — it is a tool, like `fx_cost`. Cost numbers in PERFORMANCE.md §4d, impact numbers in todo.md | `Tests/Engine/scoring_cost.gd` |
| **E2** | **The straight search has no cap** | `PLAN.md` §1.5 enumerates one position per mixed class as a cartesian product. Product is 1 when nothing merged, so the un-modded path is untouched. Bounded in practice by thirteen positions. **Now measured (E1):** on 30 cards a rank-merging rule costs 14.5 ms per scored line and merging PLUS extra rank values 27.7 ms, against 9.1 ms unmodded — a real multiplier, not a runaway. If a board ever does make it runaway, that is a gap to file, not a bound to invent. ⚠ A 23 s scoring pass once looked like that case arriving; it was an implementation bug instead (`gaps/GAP-002.md`, withdrawn) — a class's `member_keys` leaked in values that belonged to a card's OTHER class, so classes counted as `mixed` that had not been merged. `test_mod_fuzz.gd` prints the slowest pass every run, and `test_comparator.gd` section 11 asserts the assignment count directly, because that failure was invisible in results — every score was correct, just astronomically slow | `Scoring.MultiStraightHandler` |
| **E3** | **Profile rebuilt several times per scored line** | Q57(a). The straight and flush handlers consume their working profile destructively via `remove_card`, and each now also builds an unconsumed CLASSIFICATION profile for `is_flush`, so sharing one needs a cheap `HandProfile.duplicate()`. ⚠ **It is no longer only a cost.** The straight handlers build profiles from SUBSETS of the hand (one suit's cards at a time), and stage 0 closes over the keys actually present — so a subset can produce a different partition from the full hand's, and a run formed under one is classified against the other. Measured while writing `test_mod_fuzz.gd`: this is why invariant 8 asserts the position model rather than the finished meld. ⚠ **E1 now sizes it**: a scored line costs 9.1 ms on 30 cards unmodded (5.0 ms before this work — E6), and `Game.score_line` runs per row AND column while `skill_eval_poker_best` scores both again from inside — so a 5x5 board is ~20 such calls — tens to hundreds of ms of submit, an upper bound because the bench runs under the uncached `FakeEnvironment`. This is the biggest lever in the scoring path | `Scoring.PokerHands.score`, `MultiStraightHandler`, `MultiFlushHandler` |
| **E4** | **SE4 — single-walk `_scan_wrap`** | The scan restarts from every rank where one pass could find the longest wrap run. Pre-existing and unrelated to this work. E1 now exists, so this is measurable rather than speculative — but measure before optimising: the wrap scan is one term inside a call that costs 9.5 ms on 30 cards, and E3 is the larger multiplier | `Scoring.MultiStraightHandler._scan_wrap` |
| **E6** | ⚠ **The IDENTITY path costs ~2x what it did before this work** | `MEASURED` after E1 existed, current tree vs a `HEAD~1` worktree, one session one box, two runs each: 30 cards with no rules **5.01 ms → 9.15 ms**, and 1.9–2.7x at every smaller size. **This is the path every real game takes** — no shipped card implements a meld hook — so it is not a cost that waits for content. C4's safety argument ("byte-identical, zero dispatch") was only ever verified for RESULTS. ⚠ **Not diagnosed, only measured.** Suspects in order: the extra unconsumed CLASSIFICATION profile per consuming handler (`ASSUMPTIONS.md` S14) which is strictly more builds per line than before; `RankMap.find()` linear scans replacing a `Dictionary` lookup; O(k²) `has()` walks in `distinct_keys()` / `position_count()`; per-call `begin_pass`/`end_pass` churn, which the doubled 5-card floor points at. **Owner call whether ~2x on the un-modded path is acceptable** — numbers in PERFORMANCE.md §4d | `Scoring._build_profile`, `RankMap`, `PokerHands.score` |
