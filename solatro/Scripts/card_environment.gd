...
#⚠ Emitted once per skill_spotlight_check() sweep that saw any transition, carrying every card
#that transitioned in it - ONE cue covering all of them, not one per card - and only for cards
#whose skill implements on_spotlight, anything else having nothing to announce.

#A card that was ALREADY spotlit never appears here, which is also why loading a save emits
#nothing: `spotlit` is @export_storage, so a card saved spotlit loads spotlit and transitions
#nothing. No suppression code exists, because the case cannot arise.

## One or more cards just TRANSITIONED into the spotlight: the momentary activation cue.
signal spotlight_cued(cards: Array[CardData])

#Emitted by Game._spotlight_section() as each section takes the light, and once more empty from
#Game._release_spotlight().

#⚠ THIS IS NOT `spotlight_cued`, AND CONFLATING THE TWO IS A KNOWN TRAP. `spotlight_cued` says
#which cards have a TALENT to announce, filtered to skills implementing on_spotlight so a card with
#nothing to show does not flash.

#This one says which cards are BEING SCORED, and is filtered by nothing: a scored row is mostly
#plain numeral cards with no skill at all, and the beam lights the row.
...
#⚠ THE SET REPLACES, IT DOES NOT ACCUMULATE. An empty array is what retires the light and lowers
#the dim; there is deliberately no separate "stop" signal, because a second way to lower the dim
#can disagree with the light set.

## THE SCORING BEAM'S MEMBERSHIP: every card in the section being scored, or EMPTY on release.
signal spotlight_section_changed(cards: Array[CardData])

#The beat where the spotlight and the dim fade out. Owner: *"spotlight + dim occurs as cards of
#section get revealed, with both spotlight and dim effect fading away as scoring starts to happen.
#When next section is revealed, spotlight and dim effect are visible again, moving to new location"*.

#⚠ THE LIGHTS ARE NOT RETIRED BY THIS, ONLY HIDDEN. spotlight_section_changed owns the light
#SET and this owns its VISIBILITY. Keeping the set alive across the fade is what lets the next
#section TRAVEL from these positions rather than respawn at new ones, which the charts forbid.

## The section's REVEAL is over and its scoring is about to happen.
...
#One call per mod invoked and per prop slot entry. A no-op in base environments, the map and tests.

## Elapsed-processing accounting hook, which Game overrides to feed the runaway event cap.
...
#Dispatch is INSTANCE-based: each environment runs mods over its own collections. CURRENT is only
#the "environment on screen" pointer used at the boundaries, never inside dispatch.

#⚠ A MARK IS A CELL WEARING A COPIED FACE, AND THE FACE IS NOT ON THE BOARD: its rank, suit,
#stamp and skill answer the two mark hooks and nothing else, while the cell's own type goes on
#ruling its square. THE exclusion, written once - every board-wide walk below takes its mods here.
...
#⚠ THE GATE: on a cacheable environment a hook nothing on the board implements makes the walk a
#pure no-op scan, so it is skipped. Base envs, tests and map, carry no revision key and always
...
## Base environments, tests and map, are uncacheable: their collections mutate freely.
func _revision_key() -> Array:
	return []

#TODO(non-card rule sources): The Fire Marshal is a TOWN HAZARD, a modifier with no board card to
#live on, and this walk only ever visits CardDataIterator, which is board and rules cards. It needs
#a run-level modifier list the iterator also visits, plus a ruling on whether undo rewinds it.
...
#THE COMPARATOR SURFACE'S DISPATCH.

#⚠ The verdict cache is NOT here: it lives on PipComparator and is scoped to the scoring pass
#(owner ruling). The implementer cache below is a different question with a different lifetime -
#"does anything implement this hook", not "what did it answer".

#⚠ Never mutate it.

## Shared empty result, so the common "nothing implements this" answer costs no allocation.
const _NO_IMPLEMENTERS : Array[CardModifier] = []

#⚠ THE ONE WALK EVERY HELPER BELOW SHARES - the spotlit gate lived in four copies before. A
#rule applies whether or not its own card is in the hand.

## Every implementer of `hook` that may act right now: board order, unspotlit skills dropped.
...
#⚠ HOIST OUT OF PER-CARD AND PER-ITERATION LOOPS. It is a dictionary lookup on Game, but base
#environments, tests and map, return an empty revision key, so _compare_implementers caches nothing
#and WALKS EVERY BOARD CARD.
func has_implementer(hook: StringName) -> bool:
	return not _compare_implementers(hook).is_empty()

#When no, profiling takes the identity path: zero dispatches, byte-identical to the buckets this
#replaced.

## Does either pass of this situation have an implementer?
func any_pair_implementer(deny: StringName, allow: StringName) -> bool:
	return has_implementer(deny) or has_implementer(allow)

#The shape for hooks whose answer is not a boolean pass, such as the wrap bounds.
...
#Extra rank values are class MEMBERSHIPS, not a scalar verdict, so a second card offering another
#value must not be silenced by the first.
...
#⚠ A MARK ANSWERS FALSE: a marked cell is a square wearing a copied face and never a card in
#play, so a grouping rule cannot pull one into a meld to score it. The modifier half of the same
#rule is _dispatch_mods; the meaning of "marked" is BoardPlan's, so neither spells it for itself.

#THE check that lets a grouping rule PULL a board card into a meld while refusing to let it INVENT
#one - the refusal that keeps multiplicity out of scope rather than reachable sideways.

## Is this CardData somewhere in this environment's collections?
...
## ONE pass of the two-pass sameness question: the FIRST true answers and STOPS the pass.
...
#on_spotlight and on_unspotlight on the EDGE only: a card already spotlit is not re-announced.

## THE activation sweep.
func skill_spotlight_check() -> void:
#The momentary cue: every card that transitioned to spotlit during THIS sweep and has something to
#announce. ONE cue covers all of them, so it is collected across the walk and emitted once at the
#end, not per card.
...
#Only a skill with an on_spotlight hook has anything to show.
					cued.append(data)
...
#tick's three-phase pass asks rather than fires, so this path charges no processing.
...
#One hook, two recipients: a MARK's copied modifiers and the card that covered it. A mark's skill
#is carried in, never being spotlit, so the gate would silence it; a real card keeps the rule.
...
#⚠ A QUESTION IS NOT AN EFFECT FIRING (owner: *"it shouldnt trigger on checks, but only when
#effect actually triggers"*), so the per-card path charges nothing. `counts_as_activation` is what a
#caller dispatching a real effect passes, charging the ramp, the runaway cap and the combo alike.
