...
func _rand_game_data(rng: RandomNumberGenerator) -> GameData:
	...
	gs.entrance_stocks()[0].datas.assign(_rand_card_array(rng, rng.randi_range(0, 8), CardData.Stage.DRAW))
...
func _diff_game(e: Array[String], path: String, a: GameData, b: GameData) -> void:
	...
	_diff_cards(e, "%s.entrance_stocks" % path, a.all_stock_cards(), b.all_stock_cards())
