...
| S4 `WallPacker` + `PictureRect` | done | 13 rows green (P1, P2′, P3′, P4′, P5–P13). Purity verified: **0** `SettingsManager`/`get_tree`/`Engine` refs in `wall_packer.gd`. `ring` gone with no stragglers; `home_id` now used (4 refs) so home takes the centre. | `Scripts/Wall/{wall_packer,picture_rect,picture_entry}.gd` | P2′/P3′ assertions reworded — the overseer's original fixtures were unsatisfiable under the model | GAP-009 (answered) |
...
`Wall` must set `get_tree().paused = false` immediately after `add_child()`. ⚠ **S12 owes U1
("the tree is paused and stays paused") and must not be written in a way this workaround
invalidates** — U1 has to assert against a `Wall` whose pause it does not then undo.
