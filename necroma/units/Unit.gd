...
enum {
	IDLE, #stays still and doesn't attack
	MOVE, #attempts to attack after moving
	ATTACK #will repeatedly attack anything that moves into range
}
var state : int = MOVE
...
func _ready() -> void:
	...
	self.connect('moved', $"../..", '_on_Unit_moved')
...
func move_state() -> void:
	...
	#extremely fancy jump animation
	...
	jump.tween_property(self, "scale", default_scale*Vector2(1.05,0.95), duration/4)
	jump.set_ease(Tween.EASE_IN)
	jump.tween_property(self, "scale", default_scale, duration/4)
